...
function emitConditionPrisma(
  ...
  if (!field.includes('.')) {
    ...
    if (cmp !== undefined) return (cmp === null ? { [field]: null } : { [field]: cmp }) as Prisma.AddressWhereInput;
  } else {
    const cmp = scalarComparator(fieldType, op as AnyOp, v as any);
    if (cmp !== undefined) {
      const tail = field.slice(field.indexOf('.') + 1);
      const leaf = cmp === null ? null : { [tail]: cmp } as Record<string, unknown> | null;
      return nestRelWhere(relMeta, field, leaf) as Prisma.AddressWhereInput;
    }
  }

  // Unknown/custom op → return undefined (caller will try registry/custom path)
  return {} as unknown as Prisma.AddressWhereInput;
}
...
function fromIRPrisma(relMeta: RelationMeta, ir: IR): Prisma.AddressWhereInput {
  ...
  if (op === 'between') {
    ...
    if (!f.includes('.')) {
      return { AND: [{ [f]: { gte: b.a } }, { [f]: { lte: b.b } }] } as Prisma.AddressWhereInput;
    }
  ...
  if (op === 'is_null') {
    return f.includes('.') ? nestRelWhere(relMeta, f, null) as Prisma.AddressWhereInput
                           : ({ [f]: null } as Prisma.AddressWhereInput);
  }
  if (op === 'is_not_null') {
    const leaf = f.includes('.') ? f.split('.')[1]! : f;
    const payload = { [leaf]: { not: null } } as Record<string, unknown>;
    return f.includes('.') ? nestRelWhere(relMeta, f, payload) as Prisma.AddressWhereInput
                           : ({ [f]: { not: null } } as Prisma.AddressWhereInput);
  }
  ...
  if (!f.includes('.')) {
    return { [f]: { [key]: val } } as Prisma.AddressWhereInput;
  }
  const leafName = f.split('.')[1]!;
  return nestRelWhere(relMeta, f, { [leafName]: { [key]: val } }) as Prisma.AddressWhereInput;
}
...
function buildRelationWhereMap(
  meta: Readonly<FilterableMap>,
  filter?: FilterInput,
): Partial<AddressRelationWhereMap> {
  if (!filter) return {};
  const out: Partial<AddressRelationWhereMap> = {};
...
function buildSelect(
  select: readonly AddressSelectField[] | undefined,
  relWhere: Partial<AddressRelationWhereMap>,
): Prisma.AddressSelect | undefined {
  if (!select || select.length === 0) return undefined;

  const obj: Prisma.AddressSelect = {};
  const scalar: string[] = [];
...
function buildOrderBy(
  sorts: ReadonlyArray<{ field: AddressSelectField; direction?: 'asc' | 'desc' }> | undefined,
  relMeta: RelationMeta,
): Prisma.Enumerable<Prisma.AddressOrderByWithRelationInput> | undefined {
  if (!sorts || sorts.length === 0) return undefined;
  const out: Prisma.AddressOrderByWithRelationInput[] = [];
  ...
  for (const s of sorts) {
    ...
    const dot = f.indexOf('.');
    if (dot < 0) {
      out.push({ [f]: dir } as Prisma.AddressOrderByWithRelationInput);
      continue;
    }
    ...
    if (meta?.kind === 'one') {
      ...
      out.push({ [root]: { [leaf]: dir } } as unknown as Prisma.AddressOrderByWithRelationInput);
    }
  }
...
export async function resolveAddress<T extends object, S extends readonly AddressSelectField[] | undefined = readonly AddressSelectField[] | undefined>(
  ...
  filter?: FilterInput,
  custom?: CustomOpRegistry,
  options?: AddressResolveOptions<S> & { shape: 'entity' },
...
export async function resolveAddress<T extends object, S extends readonly AddressSelectField[] | undefined = readonly AddressSelectField[] | undefined>(
  ...
  // Where (top-level)
  const where: Prisma.AddressWhereInput = filter
  ...
  const rows = await ctx.client.address.findMany(args);

  if (shape === 'entity') return rows;
  // Prisma already projects by select → reuse payload as "plain"
  return rows as unknown as AddressPlain<S, T>[];
}
