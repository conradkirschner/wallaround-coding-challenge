...
import type { Expr } from './<%= entity %>FilterQuery';
...
export const <%= entity %>_OPS = <%- JSON.stringify(ALL_OPS) %> as const;
...
function fromIR(ir: IR): Expr {
  ...
  <% if (hasOp('eq')) { %> if (ir.op === 'eq')  return dotted ? nest(f, ir.value as V) : wrapField(f, ir.value as V); <% } %>
  <% if (hasOp('neq')) { %> if (ir.op === 'neq') return dotted ? nest(f, { $ne: ir.value as V } as V) : wrapField(f, { $ne: ir.value as V } as V); <% } %>
  <% if (hasOp('gt')) { %> if (ir.op === 'gt')  return dotted ? nest(f, { $gt: ir.value as V } as V) : wrapField(f, { $gt: ir.value as V } as V); <% } %>
  <% if (hasOp('gte')) { %> if (ir.op === 'gte') return dotted ? nest(f, { $gte: ir.value as V } as V) : wrapField(f, { $gte: ir.value as V } as V); <% } %>
  <% if (hasOp('lt')) { %> if (ir.op === 'lt')  return dotted ? nest(f, { $lt: ir.value as V } as V) : wrapField(f, { $lt: ir.value as V } as V); <% } %>
  <% if (hasOp('lte')) { %> if (ir.op === 'lte') return dotted ? nest(f, { $lte: ir.value as V } as V) : wrapField(f, { $lte: ir.value as V } as V); <% } %>
  <% if (hasOp('in')) { %> if (ir.op === 'in')  return dotted ? nest(f, { $in: ir.values as readonly V[] } as V) : wrapField(f, { $in: ir.values as readonly V[] } as V); <% } %>
  <% if (hasOp('between')) { %>
  if (ir.op === 'between') {
  ...
  <% } %>
  <% if (hasOp('contains')) { %> if (ir.op === 'contains')    return dotted ? nest(f, { $like: `%${likeEscape(String(ir.value))}%` } as V) : wrapField(f, { $like: `%${likeEscape(String(ir.value))}%` } as V); <% } %>
  <% if (hasOp('starts_with')) { %> if (ir.op === 'starts_with') return dotted ? nest(f, { $like: `${likeEscape(String(ir.value))}%` } as V) : wrapField(f, { $like: `${likeEscape(String(ir.value))}%` } as V); <% } %>
  <% if (hasOp('ends_with')) { %> if (ir.op === 'ends_with')   return dotted ? nest(f, { $like: `%${likeEscape(String(ir.value))}` } as V) : wrapField(f, { $like: `%${likeEscape(String(ir.value))}` } as V); <% } %>
  <% if (hasOp('is_null')) { %> if (ir.op === 'is_null')       return dotted ? nest(f, null as V) : wrapField(f, null as V); <% } %>
  <% if (hasOp('is_not_null')) { %> if (ir.op === 'is_not_null') return dotted ? nest(f, { $ne: null } as V) : wrapField(f, { $ne: null } as V); <% } %>
...
function emitCondition(
  ...
  if (field.includes('.')) {
    <% if (hasOp('eq')) { %> if (name === 'eq')  return nest(field, v as V); <% } %>
    <% if (hasOp('neq')) { %> if (name === 'neq') return nest(field, { $ne: v as V } as V); <% } %>
    <% if (hasOp('gt')) { %> if (name === 'gt')  return nest(field, { $gt: v as V } as V); <% } %>
    <% if (hasOp('gte')) { %> if (name === 'gte') return nest(field, { $gte: v as V } as V); <% } %>
    <% if (hasOp('lt')) { %> if (name === 'lt')  return nest(field, { $lt: v as V } as V); <% } %>
    <% if (hasOp('lte')) { %> if (name === 'lte') return nest(field, { $lte: v as V } as V); <% } %>
    <% if (hasOp('in')) { %> if (name === 'in')  return nest(field, { $in: v as readonly V[] } as V); <% } %>
    <% if (hasOp('between')) { %>
    if (name === 'between') {
    ...
    <% } %>
    <% if (hasOp('contains')) { %> if (name === 'contains')    return nest(field, { $like: `%${likeEscape(String(v))}%` } as V); <% } %>
    <% if (hasOp('starts_with')) { %> if (name === 'starts_with') return nest(field, { $like: `${likeEscape(String(v))}%` } as V); <% } %>
    <% if (hasOp('ends_with')) { %> if (name === 'ends_with')   return nest(field, { $like: `%${likeEscape(String(v))}` } as V); <% } %>
    <% if (hasOp('is_null')) { %> if (name === 'is_null')       return nest(field, null as V); <% } %>
    <% if (hasOp('is_not_null')) { %> if (name === 'is_not_null') return nest(field, { $ne: null } as V); <% } %>
  } else {
    <% if (hasOp('eq')) { %> if (name === 'eq')  return wrapField(field, v as V); <% } %>
    <% if (hasOp('neq')) { %> if (name === 'neq') return wrapField(field, { $ne: v as V } as V); <% } %>
    <% if (hasOp('gt')) { %> if (name === 'gt')  return wrapField(field, { $gt: v as V } as V); <% } %>
    <% if (hasOp('gte')) { %> if (name === 'gte') return wrapField(field, { $gte: v as V } as V); <% } %>
    <% if (hasOp('lt')) { %> if (name === 'lt')  return wrapField(field, { $lt: v as V } as V); <% } %>
    <% if (hasOp('lte')) { %> if (name === 'lte') return wrapField(field, { $lte: v as V } as V); <% } %>
    <% if (hasOp('in')) { %> if (name === 'in')  return wrapField(field, { $in: v as readonly V[] } as V); <% } %>
    <% if (hasOp('between')) { %>
    if (name === 'between') {
      const [a, b] = v as readonly [Primitive, Primitive];
    ...
    <% } %>
    <% if (hasOp('contains')) { %> if (name === 'contains')    return wrapField(field, { $like: `%${likeEscape(String(v))}%` } as V); <% } %>
    <% if (hasOp('starts_with')) { %> if (name === 'starts_with') return wrapField(field, { $like: `${likeEscape(String(v))}%` } as V); <% } %>
    <% if (hasOp('ends_with')) { %> if (name === 'ends_with')   return wrapField(field, { $like: `%${likeEscape(String(v))}` } as V); <% } %>
    <% if (hasOp('is_null')) { %> if (name === 'is_null')       return wrapField(field, null as V); <% } %>
    <% if (hasOp('is_not_null')) { %> if (name === 'is_not_null') return wrapField(field, { $ne: null } as V); <% } %>
  }
...
function buildWhere(
  ...
  const allowed = def.operators.filter((o): o is Operator => (<readonly string[]><unknown><%= entity %>_OPS).includes(o));
