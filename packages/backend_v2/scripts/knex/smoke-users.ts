// scripts/knex/smoke-users.ts
import 'reflect-metadata';
import type { FilterInput } from '../../src/filtering/ast';
import { User } from '../../src/domain';

// Knex bootstrap (DB + ctx + capabilities)
import { sqlCtx, db } from './bootstrap.sql';

// auto-generated (Knex adapter)
import { resolveUser as resolveUsers } from '../../src/generated/knex/UserResolver';
...
async function main() {
    ...
    const results1 = await resolveUsers(sqlCtx, User, filter1, undefined, {
    ...
    const results2 = await resolveUsers(sqlCtx, User, filter2, undefined, {
    ...
    const results3 = await resolveUsers(sqlCtx, User, filter3, undefined, {
    ...
    const results4 = await resolveUsers(sqlCtx, User, filter4, undefined, {
    ...
    const results5 = await resolveUsers(sqlCtx, User, { field: 'address.street2', op: 'is_null' }, undefined, {
    ...
    if (results5[0]?.address) {
        ...
        console.log('   address object keys =', Object.keys(a));
    }
    ...
    const page1 = await resolveUsers(sqlCtx, User, paginatedFilter, undefined, {
        query: { select: select6, sort: [{ field: 'createdAt', direction: 'asc' }], limit: 1, offset: 0 },
        security: { requireSelectableForFilter: true },
    });
    const page2 = await resolveUsers(sqlCtx, User, paginatedFilter, undefined, {
    ...
    try {
        ...
        await resolveUsers(sqlCtx, User, bad2, undefined, {
            query: { select: ['email'] as const },
            security: { requireSelectableForFilter: true },
        });
    ...
    // 8) 1:n relation (posts) — default quantifier (SOME) — title contains 'Hello' AND published == true
    ...
    const results8 = await resolveUsers(sqlCtx, User, filter8, undefined, {
    ...
    if ((results8 as any)[0]?.posts) {
        const p0 = (results8 as any)[0].posts;
        console.log('   first user posts keys =', Array.isArray(p0) ? Object.keys(p0[0] ?? {}) : Object.keys(p0 ?? {}));
    }
    ...
    const results9 = await resolveUsers(sqlCtx, User, filter9, undefined, {
        query: { select: select9, sort: [{ field: 'email', direction: 'asc' }] },
        security: { requireSelectableForFilter: true },
        // SQL adapter usually returns "plain" objects; entity shape is not applicable
    });
