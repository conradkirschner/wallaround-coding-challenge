// tests/unit/schema.defaults.test.ts
...
describe('schema – defaults when no operatorMap is provided', () => {
  it('applies sensible defaults per type', () => {
    ...
    expect(sorted(findOperator(schema, 'between')!.supportedTypes))
      .toEqual(['date', 'number']);

    // boolean defaults include eq/neq; eq spans all scalar types by default
    expect(sorted(findOperator(schema, 'eq')!.supportedTypes))
      .toEqual(['boolean', 'date', 'number', 'string']);
    expect(sorted(findOperator(schema, 'neq')!.supportedTypes))
      .toEqual(['boolean', 'date', 'number', 'string']);
