...
export function useRequestTester({ api, encoded, setRows }: Params) {
    ...
    const getUrlPreview = React.useMemo(() => {
        ...
        try { return api.withFilterInUrl(url, encoded); } catch { return null; }
    }, [method, url, api, encoded]);

    const resetBody = () => { setBodyDirty(false); setBodyDraft(pretty({ filter: encoded })); };
    ...
    const send = async () => {
        ...
        try {
            ...
            if (method === 'GET') {
                ...
                let payload: unknown;
                try { payload = JSON.parse(bodyDraft); }
                catch (e) { throw new HttpRequestError(`POST body is not valid JSON: ${(e as Error).message}`); }
                res = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
            }

            if (!res.ok) {
                const text = await res.text().catch(() => '');
                throw new HttpRequestError(`HTTP ${res.status} ${res.statusText}${text ? ` – ${text}` : ''}`, res.status);
            }
    ...
    return {
        method, setMethod,
        url, setUrl,
        bodyDraft, setBodyDraft, bodyDirty, setBodyDirty,
        loading, error, success,
        getUrlPreview, resetBody, send,
    };
}
