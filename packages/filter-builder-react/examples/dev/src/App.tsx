    ...
    <div className="mx-auto max-w-6xl p-4 space-y-6" data-test-id="app-root">
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <JsonEditorCard
          ...
          extraActionsLeft={
            <>
              <span className="text-[10px] text-gray-500 mr-1" aria-hidden>
                Presets:
              </span>
              <button
                type="button"
      ...
      <section className="space-y-3">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <Card title="GET Query Param" data-test-id="query-param" ariaLabel="GET Query Param">
            <code className="break-words text-xs" aria-label="Query parameter">
              {evalState.qp}
            </code>
            <div className="mt-2 text-xs" aria-live="polite">
              <span
                className={evalState.validation.valid ? 'text-green-700' : 'text-red-700'}
                data-test-id="validation-summary"
                aria-label={evalState.validation.valid ? 'Filter valid' : 'Filter invalid'}
              >
                {evalState.validation.valid ? 'Valid' : 'Invalid'} (
                {evalState.validation.issues.length} issues)
              </span>
