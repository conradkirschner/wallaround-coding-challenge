    ...
    return (
        <Card title="Request Tester" data-test-id={testId} ariaLabel="Request Tester">
            <div className="flex items-center gap-2 mb-2">
                <label className="text-xs text-gray-600" htmlFor="req-url">URL</label>
                <input
                    id="req-url"
                    type="text"
                    value={url}
            ...
            {method === 'GET' && !!getUrlPreview && (
                <div className="mb-2">
                    ...
                    <code className="break-words text-[11px]" data-test-id={testId ? `${testId}__get-preview` : undefined}>
                        {getUrlPreview}
                    </code>
                </div>
            )}
            ...
            {method === 'POST' && (
                <div className="mb-2">
                    <div className="flex items-center justify-between mb-1">
                        <label className="text-[11px] text-gray-600" htmlFor="req-body">POST Body (JSON)</label>
                        <div className="flex items-center gap-2">
                            {isBodyDirty ? (
                                <span className="text-[11px] text-amber-700" data-test-id={testId ? `${testId}__dirty-hint` : undefined}>
            ...
            <div className="min-h-[1.25rem]" aria-live="polite">
                {error ? (
                    <p className="text-xs text-red-700 whitespace-pre-wrap" data-test-id={testId ? `${testId}__error` : undefined} role="alert">
                        {error}
                    </p>
                ) : success ? (
                    <p className="text-xs text-green-700" data-test-id={testId ? `${testId}__success` : undefined}>
                        {success}
                    </p>
                ) : null}
            </div>
