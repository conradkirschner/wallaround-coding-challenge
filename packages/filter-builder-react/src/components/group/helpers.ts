...
export const isCondition = (n: FilterNode): n is ConditionNode => 'field' in n && 'operator' in n;

/** Group discriminators. */
export const getKind = (node: GroupNode): GroupKind => ('and' in node ? 'and' : 'or');

export const getChildren = (node: GroupNode): ReadonlyArray<FilterNode> =>
  'and' in node ? node.and : node.or;

export const makeGroup = (kind: GroupKind, children: ReadonlyArray<FilterNode>): GroupNode =>
