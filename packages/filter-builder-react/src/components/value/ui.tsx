...
/** Renders the single-value UI based on the discriminated union. */
        ...
        case 'boolean':
            return (
                <div className="flex items-center gap-2" data-test-id={testId}>
                    <label htmlFor={h.controlId} className="sr-only">{h.ariaLabel}</label>
                    <h.Checkbox id={h.controlId} checked={h.checked} onChange={h.onBool} aria-label={h.ariaLabel} />
                </div>
            );
        case 'select':
            return (
        ...
        case 'number':
            return (
                <div className="min-w-[10rem]" data-test-id={testId}>
                    <label htmlFor={h.controlId} className="sr-only">{h.ariaLabel}</label>
                    <h.NumberInput id={h.controlId} value={h.value} onChange={h.onNumber} aria-label={h.ariaLabel} />
                </div>
            );
        case 'date':
            return (
                <div className="min-w-[12rem]" data-test-id={testId}>
                    <label htmlFor={h.controlId} className="sr-only">{h.ariaLabel}</label>
                    <h.DateInput id={h.controlId} value={h.value} onChange={h.onDate} aria-label={h.ariaLabel} />
                </div>
            );
        case 'string':
            return (
